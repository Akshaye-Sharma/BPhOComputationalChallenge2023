...
def this():
    global planet1
    global planet2
    p1 = (planet1_entry.get()).lower()
    p2 = (planet2_entry.get()).lower()
    if p1 == "mercury":
        chosen_planets.append(0)
    if p1 == "venus":
        chosen_planets.append(1)
    if p1 == "earth":
        chosen_planets.append(2)
    if p1 == "mars":
        chosen_planets.append(3)
    if p2 == "mercury":
        chosen_planets.append(0)
    if p2 == "venus":
        chosen_planets.append(1)
...
login_label = tk.Label(frame, text="Login", fg='white')
planet1_label = tk.Label(frame, text="Planet 1")
planet1_entry = tk.Entry(frame)
planet2_entry = tk.Entry(frame)
planet2_label = tk.Label(frame, text="Planet 2")
login_button = tk.Button(frame, text="Plot!", command=this)

# Placing widgets on the screen
login_label.grid(row=0, column=3, columnspan=2, sticky="news", pady=40)
planet1_label.grid(row=2, column=3)
planet1_entry.grid(row=2, column=4, pady=20)
planet2_label.grid(row=3, column=3)
planet2_entry.grid(row=3, column=4, pady=20)
login_button.grid(row=4, column=3, padx=20, columnspan=2)
frame.pack()
...
if planets[planet1][3] > planets[planet2][3]: # Finding larger orbital period
    max_num = planets[planet1][3]
else:
    max_num = planets[planet2][3]
